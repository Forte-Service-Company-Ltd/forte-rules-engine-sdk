...
test('Can catch all missing required fields in foreign call JSON', () => {
  ...
  if (isLeft(parsedFC)) {
    ...
    expect(errors[6].message).toEqual('Invalid input: expected string, received undefined: Field callingFunction')
  }
})
...
test('Can catch all wrong inputs for fields in foreign call JSON', () => {
  ...
  if (isLeft(parsedFC)) {
    ...
    expect(errors[5].message).toEqual('Invalid input: expected string, received number: Field mappedTrackerKeyValues')
    expect(errors[6].message).toEqual('Invalid input: expected string, received number: Field callingFunction')
  }
})
...
test('Fails validation when function has undefined parameters (no parens)', () => {
  ...
  if (isLeft(parsed)) {
    ...
    expect(errors[0].message).toEqual('Unsupported argument type: Field function')
  }
})
...
test('Can catch all missing required fields in tracker JSON', () => {
  ...
  if (isLeft(parsedTracker)) {
    ...
    expect(errors[2].message).toEqual('Invalid input: Field initialValue')
  }
})
...
test('Can return multiple errors if tracker JSON is invalid', () => {
  ...
  if (isLeft(parsedTracker)) {
    ...
    expect(errors.length).toEqual(2)
    expect(errors[0].message).toEqual('Invalid input: expected string, received number: Field name')
    expect(errors[1].message).toEqual('Invalid input: Field initialValue')
  }
})
...
test('Can catch missing calling function in foreign call in policy JSON', () => {
  ...
  if (isLeft(parsedPolicy)) {
    ...
    expect(errors[0].message).toEqual('Invalid reference call')
  }
})
...
test('Can catch all missing required fields in policy JSON', () => {
  ...
  if (isLeft(parsedPolicy)) {
    ...
    expect(errors[0].message).toEqual('Invalid input: expected string, received undefined: Field PolicyType')
    expect(errors[1].message).toEqual('Invalid input: expected array, received undefined: Field CallingFunctions')
    ...
    expect(errors[4].message).toEqual('Invalid input: expected array, received undefined: Field MappedTrackers')
...
test('Can catch all wrong inputs for fields in policy JSON', () => {
  ...
  if (isLeft(parsedPolicy)) {
    ...
    expect(errors[6].message).toEqual('Invalid input: expected array, received string: Field Rules')
  }
})
